# Chrome Web Store submission guide — PromptLint v1.0.0
...
Upload **`promptlint-v1.0.0-store.zip`** (built from `promptlint/` with `manifest.json` at the **root of the zip** — the Web Store rejects zips where the manifest is nested inside a folder).

Rebuild it any time with:

```bash
cd promptlint && zip -r ../promptlint-v1.0.0-store.zip . -x "README.md" -x "DECISIONS.md"
```
...
## 5. Pre-flight checklist (verified for v1.0.0)
